...
partial class DragSortRow
{
    ...
    /// <remarks>
    /// This is one of the two reasons drag sort never worked on Android. A ScrollView calls
    ...
    /// chain not to intercept. The framework resets the flag on the next ACTION_DOWN, so it only
    /// ever covers the one gesture that started on a drag handle; touches anywhere else in the
    /// table still scroll normally.
    /// </remarks>
    ...
    {
        // bringChildToFront only shuffles the parent's child array - unlike the remove/re-add
        // that MAUI's ZIndex performs, it never detaches the view, so the in-flight touch lives.
        if (raised && this.Handler?.PlatformView is AView native)
            native.BringToFront();
    }
