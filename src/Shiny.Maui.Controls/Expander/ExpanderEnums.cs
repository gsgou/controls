...
/// <c>Height | Fade</c>, and XAML writes it the way it writes any other flags enum:
/// <c>Animation="Height,Fade"</c>.
/// </summary>
[Flags]
public enum ExpanderAnimation
{
    ...
    /// <summary>
    /// Grow and shrink the panel between zero and the content's measured size, so the views below it
    /// move with the reveal instead of being uncovered by it. This is what makes an accordion read as
    /// an accordion.
    /// </summary>
    Height = 4
}
...
{
    /// <summary>Enters downwards, from above — the natural match for a header-on-top expander.</summary>
    Top,
...
public enum ExpandDirection
{
    ...
    /// <summary>Content above the header — what a panel anchored to the bottom of a page wants.</summary>
    Up
}
...
public enum ExpanderIndicatorPosition
{
    ...
    /// <summary>Leading edge, before the header content — the disclosure-triangle look.</summary>
    Start
}
...
public enum ExpanderIndicatorMode
{
    ...
    /// <summary>One glyph (<see cref="Expander.CollapsedIcon"/>) that rotates a quarter turn when expanded.</summary>
    Rotate,

    /// <summary>Two glyphs, swapped between <see cref="Expander.CollapsedIcon"/> and <see cref="Expander.ExpandedIcon"/>.</summary>
    Swap
}
