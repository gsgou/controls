...
public static class DockingServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddDockPanel<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] TComponent>(
        this IServiceCollection services, string panelTypeId)
        where TComponent : ComponentBase
    {
        services.AddSingleton<IDockableContentFactory>(_ => new ComponentPanelFactory<TComponent>(panelTypeId));
        return services;
    }
    ...
    {
        public ComponentPanelFactory(string panelTypeId) => PanelTypeId = panelTypeId;
