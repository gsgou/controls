...
namespace Sample.Blazor.Chat;

/// <summary>
/// A fully in-memory <see cref="IChatSessionProvider"/> for the Blazor demo. Seeds a "demo" session
/// (me + Alice + Bob) with history and simulates a live conversation: outgoing sends are echoed,
/// replies arrive after a short typing burst, and reactions/edits/deletes/read-receipts raise the
/// matching events. AOT/trim-safe — no anonymous types cross the JS interop boundary.
/// </summary>
...
/// <summary>
/// An in-memory provider seeded with one conversation per Kitchen Sink contact. The page binds the
/// embedded ChatView to this provider and swaps the bound session id as contacts are picked.
/// </summary>
public sealed class KitchenSinkChatProvider : IChatSessionProvider
{
    public sealed record Contact(string Id, string DisplayName, string BubbleColorHex, string? AvatarUrl, (string Content, int MinutesAgo)[] Seed);
    ...
    public KitchenSinkChatProvider()
    {
        foreach (var c in Contacts)
        {
            var users = new[]
            {
                ...
                new ChatSessionUserInfo(c.Id, c.DisplayName, c.AvatarUrl, c.BubbleColorHex, DateTimeOffset.Now.AddDays(-1))
            };
...
/// <summary>The persistent data behind a session. Lives in the provider so history survives the control re-resolving the session.</summary>
public sealed class InMemoryChatStore
{
    ...
    public static InMemoryChatStore CreateDemo(string sessionId)
    {
        ...
        var users = new[]
        {
            ...
            new ChatSessionUserInfo("alice", "Alice Johnson", "_content/Shiny.Blazor.Controls/shiny_logo.png", "#E3F2FD", joined),
            new ChatSessionUserInfo("bob", "Bob Smith", null, "#FFF3E0", joined)
        };
...
/// <summary>A live session over an <see cref="InMemoryChatStore"/>. A fresh instance is handed to the control each resolve; the store is shared.</summary>
public sealed class InMemoryChatSession : IChatSession
{
