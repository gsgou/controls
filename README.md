...
Live preview with zoom / torch / lens selection, photo + video capture, and live color filters (Mono, Noir, Sepia, Vivid, Cool, Warm, Fade, Chrome, Instant, Tonal) applied to both the preview and captured photos. Add frame analyzers — `Shiny.Maui.Controls.Camera.Barcode`, `.Camera.Face`, `.Camera.Motion`, `.Camera.Ocr`, `.Camera.Documents` — to scan barcodes, detect faces/motion, run OCR, or extract structured documents — every document analyzer hands back a strongly-typed record with nullable fields: `Invoice` (with order lines), `Receipt` (line items, per-tax breakdown, subtotal/tip/total), `DriversLicense` (AAMVA, incl. Canadian provinces + jurisdiction), `HealthCard` (Canadian province-aware: RAMQ/OHIP/MSP/AHCIP…), `CreditCard`, and `Passport` (MRZ). Each analyzer raises its own strongly-typed event (or a bindable `Command`), can be declared right in XAML, added/removed live or toggled on/off via `IsEnabled`, and draws styled bounding boxes via the built-in `CameraOverlayView` (`MotionAnalyzer` draws a box per distinct moving region). Document analyzers **accumulate fields across a few frames** (`AccumulationFrames`) and fire **once** with the richest merged record instead of a flood of partials, and `MotionAnalyzer` **debounces** its event (`EnterFrames`/`ExitFrames`) so it isn't twitchy. Any analyzer can **capture a still and stop the camera on detection** — set `CaptureOnDetection`/`StopOnDetection` and handle `CameraView.DetectionCaptured`, or call `CameraView.CaptureAndStopAsync()` yourself. See the [CameraView docs](https://shinylib.net/controls/cameraview/).
