...
Live preview with zoom / torch / lens selection, photo + video capture, and live color filters (Mono, Noir, Sepia, Vivid, Cool, Warm, Fade, Chrome, Instant, Tonal) applied to both the preview and captured photos. Add frame analyzers — `Shiny.Maui.Controls.Camera.Barcode`, `.Camera.Face`, `.Camera.Motion`, `.Camera.Ocr`, `.Camera.Documents` — to scan barcodes, detect faces/motion, run OCR, or extract structured documents — every document analyzer hands back a strongly-typed record with nullable fields: `Invoice` (with order lines), `Receipt` (line items, per-tax breakdown, subtotal/tip/total), `DriversLicense` (AAMVA, incl. Canadian provinces + jurisdiction), `HealthCard` (Canadian province-aware: RAMQ/OHIP/MSP/AHCIP…), `CreditCard`, and `Passport` (MRZ). Each analyzer can be declared right in XAML, added/removed live or toggled on/off via `IsEnabled`, and draws styled bounding boxes via the built-in `CameraOverlayView` (`MotionAnalyzer` draws a box per distinct moving region). Detection uses a **gated "scan trigger"** model — boxes draw continuously, but a result is delivered only after you **arm** the analyzers (`CameraView.Scan()` / `ScanCommand`, e.g. from a Fab); the next confirmed detection fires once (single-shot), and a per-analyzer `OnDetected` (`Func<TArgs, Task<bool>>`) returning `true` keeps scanning — no more event firehose. Document analyzers **accumulate fields across a few frames** (`AccumulationFrames`) and fire **once** with the richest merged record, and `MotionAnalyzer` **debounces** its event (`EnterFrames`/`ExitFrames`). Blazor mirrors this imperatively via `await camera.RequestBarcodeAsync()` off an `@ref`. For "scan then freeze", call `CameraView.CaptureAndStopAsync()` inside `OnDetected`. See the [CameraView docs](https://shinylib.net/controls/cameraview/).
